...
- [x] **Task 1: Adaugă `GetByIdAsync` în serviciu + acțiune `GetById` în controller**
  ...
  - [x] În `IMapPointService`, adaugă `Task<MapPointDto?> GetByIdAsync(string id);`.
  - [x] În `MapPointService`, implementează `GetByIdAsync`: validează `id` cu
  ...
  - [x] În `MapPointsController`, adaugă:
    ```csharp
    [HttpGet("{id}")]
    [AllowAnonymous]
  ...
  - [x] Nu adăuga `Type`/`Status` pe `MapPointDto` — rămân în afara scopului acestui task (deja
    disponibile din feature-ul MVT pe partea de client, per Context).

  LOGGING REQUIREMENTS:
  - [x] La intrare în acțiunea `GetById`: `_logger.LogInformation("GetById action called. Route:
  ...
  - [x] Dacă `id` nu e un `ObjectId` valid: `_logger.LogWarning("GetByIdAsync received an invalid
    ObjectId. Id: {Id}", id);` în serviciu — mesaj identic ca stil cu cel din `UpdateAsync`
    (`"UpdateAsync received an invalid ObjectId. Id: {Id}"`), înainte de a întoarce `null`.
  - [x] Dacă `id` e valid dar punctul nu există în DB: `_logger.LogInformation("GetByIdAsync:
    point not found. Id: {Id}", id);` în serviciu, înainte de a întoarce `null` — nivel
    `Information`, nu `Warning`, pentru că un id valid dar inexistent (ex. punct șters) e un caz
    normal la un `GET`, nu o eroare de client (spre deosebire de `UpdateAsync`, care nu are deloc
    un log pentru acest caz — aici îl adăugăm explicit pentru că setarea proiectului e
    `Logging: verbose`).
  - [x] Dacă punctul e găsit: `_logger.LogDebug("GetByIdAsync: point {Id} found", id);` în
    serviciu.

### Phase 2: Frontend — HttpClient, serviciu, selecție + highlight + panou de detalii

- [x] **Task 2: Activează `HttpClient` și creează `MapPointsService`**
  ...
  - [x] În `app.config.ts`, adaugă `provideHttpClient()` (import din `@angular/common/http`) în
    array-ul `providers`, alături de `provideBrowserGlobalErrorListeners()` și `provideRouter(routes)`.
  - [x] Creează `map-points.service.ts` ca `@Injectable({ providedIn: 'root' })`, cu:
    - [x] Constanta `private readonly apiBaseUrl = 'http://localhost:5205';`, cu același
      comentariu explicativ ca `mapPointsApiBaseUrl` din `map.ts` (nu există `environment.ts`,
      hardcodare locală e convenția stabilită).
    - [x] Un `interface MapPointDetails { id: string; name: string; description: string | null;
      longitude: number; latitude: number; }` exportat, care oglindește exact câmpurile din
      `MapPointDto` backend (Task 1).
    - [x] O metodă `getById(id: string): Observable<MapPointDetails>` care face
      `this.http.get<MapPointDetails>(`${this.apiBaseUrl}/api/mappoints/${id}`)` (ruta corectă
      identificată în Context, nu `/api/points/{id}`).
    - [x] `HttpClient` injectat prin `private readonly http = inject(HttpClient);`.

  LOGGING REQUIREMENTS:
  - [x] `console.debug('[MapPointsService] fetching point details', { id })` la începutul
    `getById`, consecvent cu stilul `[ComponentName] message {data}` deja folosit în `map.ts`.

- [x] **Task 3: Detectează click pe un punct existent, evidențiază-l și încarcă detaliile**
  Fișiere: `src/talent-map-client/src/app/map/map.ts`
  (depinde de Task 2)

  - [x] Injectează `MapPointsService` (`private readonly mapPointsService =
    inject(MapPointsService);`) și importă `MapPointDetails`.
  - [x] Adaugă un signal nou `readonly selectedPoint = signal<MapPointDetails | null>(null);`
  ...
  - [x] În `handleMapClick`, **înaintea** verificării `insideMoldova`/`showTemporaryMarker`,
  ...
  - [x] Dacă `hits.length === 0` (click-ul nu a lovit un punct existent) și există deja o
    selecție activă, curăț-o **înainte** de a continua spre gating-ul `insideMoldova`/
    `showTemporaryMarker`: `this.selectedMarker?.remove(); this.selectedMarker = undefined;
    this.selectedPoint.set(null);`. Altfel, un click ulterior pe o zonă goală (de exemplu pentru
    a plasa un `temporaryMarker` de creare a unui punct nou) ar lăsa marker-ul albastru și
    panoul de detalii din Task 4 "agățate" pe hartă, deși utilizatorul a trecut la o acțiune
    fără legătură cu punctul selectat anterior — `.set(...)` din `selectExistingPoint` curăță
    selecția doar când următorul click lovește *alt* punct existent, nu și pentru clickuri pe
    zone goale.
  - [x] Creează metoda privată `private selectExistingPoint(id: string, geometry:
    GeoJSON.Geometry): void`:
    - [x] Ghid pentru coordonate: geometry-ul feature-ului din `queryRenderedFeatures` e deja în
    ...
    - [x] Evidențiază punctul: `this.selectedMarker?.remove(); this.selectedMarker = new
      ...
      vizual).
    - [x] Apelează `this.mapPointsService.getById(id)` (RxJS `subscribe`, cu
  ...
  - [x] În `destroyRef.onDestroy`, adaugă și curățarea noului marker:
    `this.selectedMarker?.remove(); this.selectedMarker = undefined;` — oglindește exact
    curățarea existentă pentru `temporaryMarker`.
  - [x] Nu modifica stilizarea `mappoints-circle`/`mappoints-label` din `addMapPointsLayer` —
    highlight-ul se face exclusiv prin marker-ul nou, nu prin `feature-state`/paint expressions.

  LOGGING REQUIREMENTS:
  - [x] `console.debug('[MapComponent] existing point clicked', { id })` imediat ce un feature e
    detectat prin `queryRenderedFeatures`.
  - [x] `console.debug('[MapComponent] point details loaded', { id, details })` la succesul
    apelului `getById`.
  - [x] `console.error('[MapComponent] failed to load point details', { id, error })` la eroare.

- [x] **Task 4: Panou minimal de detalii legat de `selectedPoint`**
  Fișiere: `src/talent-map-client/src/app/map/map.html`, `src/talent-map-client/src/app/map/map.css`
  (depinde de Task 3)

  - [x] În `map.html`, adaugă sub `<div #mapContainer>` un bloc condiționat de `@if
  ...
  - [x] În `map.css`, adaugă stiluri minime pentru acest panou (poziționare absolută, fundal alb
  ...
  - [x] Nu adăuga buton de închidere/logica de deselectare explicită (la cerere) — în afara
    scopului acestui task. Selecția e deja curățată automat fie de un click pe alt punct existent
    (prin `.set(...)` din `selectExistingPoint`), fie de un click pe o zonă goală (prin curățarea
    adăugată în Task 3); un buton dedicat de închidere poate fi adăugat într-un task viitor.

  LOGGING REQUIREMENTS:
  - [x] Niciuna suplimentară — acest task e strict template/CSS, fără logică nouă (logging-ul
